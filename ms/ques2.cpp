...
Given an array arr[] which represents the chain of matrices such that the ith matrix Ai is of dimension arr[i-1] x arr[i]. Your task is to write a function that should print the minimum number of multiplications needed to multiply the chain.
  ...
  int matrixChainMultiplication(int arr[], int start, int end)
  {
  	...
  	int output,min = INT_MAX;
  	
  	for(int i=start;i<end;i++)
  	{
  		output = matrixChainMultiplication(arr,start,i) + matrixChainMultiplication(arr,i+1,end) + arr[start-1]*arr[i]*arr[end];
  		if(output<min)
  		{
  			min = output;
  		}
  ...
  int main()
  {
  	...
  	for(int i=0;i<testcases;i++)
  	{
  		...
  		int output = matrixChainMultiplication(arr,1,size-1);
  		cout << output << endl;
  	}	
